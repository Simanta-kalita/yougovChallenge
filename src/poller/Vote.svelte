 ...
 {:then previousVote}
   ...
   <Tile>
     <TileGroup bind:selected>
       {#each poll.answers as answer, index}
         <RadioTile
           value={index + ""}
           disabled={(!!previousVote || !!submittedVote) && !selectedVote}>{answer}</RadioTile
         >
       {/each}
     </TileGroup>
   </Tile>
   <Tile>
     <Button
       ...
       on:click={() =>
         (selectedVote = false)}
       disabled={(!selected || !!previousVote || !!submittedVote) && !selectedVote}
     >
       Vote
     </Button>
   </Tile>
